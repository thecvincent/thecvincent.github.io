          ...
          var kinetic_moving=false,
            ...
            loadImgPreview=function ($item, callback)
            {
              ...
              preloadImage(largeSrc, function ()
              {
                ...
                if(!hasImgPreview) $('#previewTmpl').tmpl(largeImageData).insertAfter($ibWrapper);
                else $('#ib-img-preview').children('img.ib-preview-img').attr('src', largeSrc).end().find('span.ib-preview-descr').text(description);
                ...
                }, 500, 'easeOutExpo', function ()
                {
                  ...
                  }, 400, function ()
                  {
                    ...
                    if(imgItemsCount> 1) $this.find('div.ib-nav').show();
                    if(callback) callback.call();
                  });
                });
                if(!hasImgPreview) initImgPreviewEvents();
              });
            },
            ...
            loadContentItem=function ($item, callback)
            {
              ...
              if(!hasContentPreview) $('#contentTmpl').tmpl(contentData).insertAfter($ibWrapper);
              //set the returned values and show/animate preview
              ...
              if(!hasContentPreview) initContentPreviewEvents();
            },
            // preloads an image
            ...
            navigate=function (dir)
            {
              ...
              preloadImage(largeSrc, function ()
              {
                ...
                //get dimentions for the image, based on the windows size
                var dim=getImageDim(largeSrc);
                $preview.children('img.ib-preview-img').attr('src', largeSrc).css(
                {
                  width: dim.width,
                  height: dim.height,
                  left: dim.left,
                  top: dim.top
                }).end().find('span.ib-preview-descr').text(description);
                $ibWrapper.scrollTop($item.offset().top).scrollLeft($item.offset().left);
                isAnimating=false;
              });
            },
